Shader "Unlit/PUCBottleShader"
{
    Properties
    {
        ...
        _WaveHeight ("Wave Height", Range(0.0, 0.02)) = 0.1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata_t v)
            {
                ...
                o.texcoord.y += _Time.y * 4; // Add time-based offset to the Y coordinate
            ...
            {
                // Add scrolling effect to the texture coordinates
                float timeOffset = _Time.z * 0.1; // Adjust the multiplier to control speed
                float2 scrollingTexCoords = i.texcoord;
                scrollingTexCoords.y += timeOffset; // Add time-based offset to the Y coordinate

                // Sample the main texture with the modified coordinates
                fixed4 col = tex2D(_MainTex, scrollingTexCoords) * _Color;
